    ...
    func updateStats() {
        if bgData.count > 0 {
            ...
            if UnitSettingsStore.shared.variabilityMetricMode == .stdDeviation {
                statsDisplayModel.variabilityTitle = "Std Dev:"
                if UnitSettingsStore.shared.glucoseUnit == .mgdL {
                    statsDisplayModel.variabilityValue = String(format: "%.0f", stats.stdDev)
                } else {
                    statsDisplayModel.variabilityValue = String(format: "%.1f", stats.stdDev)
                }
            } else {
                statsDisplayModel.variabilityTitle = "CV:"
                statsDisplayModel.variabilityValue = String(format: "%.1f%%", stats.coefficientOfVariation)
            }
